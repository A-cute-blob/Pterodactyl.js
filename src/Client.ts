 ...
 * @param {string} ops.domain The panel url
 * @example const client = new Client({ api: 'key', domain: 'example.in'})
 */
 constructor(ops: { api: String, domain: String}) {
	 super();
 ...
 */
  ...
  */
 static async server_create(ops=params, client: Client) {
...
const auth = { Bearer: client.api}
const bearer = JSON.stringify(auth)
await fetch(`${client.domain}/api/application/servers`, {
	        method: 'post',
...
const auth = { Bearer: client.api}
const bearer = JSON.stringify(auth)
await fetch(`${client.domain}/api/application/servers/${id}`, {
	        method: 'delete',
        headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', "Authorization":  bearer },

}).then(res => {
	if(res.status === 403) throw new Error('Not a valid api')
client.emit('serverDelete', res)
	return res
